...
class AuthService:
    ...
    def LoginService(db: Session, email: str, password: str) -> Dict[str, Any]:
        user = UserService.getUserByEmail(db, email)
        ...
        if not AuthService.verify_password(password, user.password):  # type: ignore
        ...
        if user.role:
            ...
            permissions_list = [
                ...
                for permission in (user.role.permissions or [])
            ]
